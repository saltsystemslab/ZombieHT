...
out_dir="sponge_perf/gzhm_variants${latency}_$1"
...
for VARIANT in "${VARIANTS[@]}"; do
  ...
  cmake . -B${build_dir}/$VARIANT -DCMAKE_BUILD_TYPE=RelWithDebugInfo -DVARIANT=$VARIANT
...
for VARIANT in "${VARIANTS[@]}"; do
  ...
  perf record numactl -N 0 -m 0 ./${build_dir}/$VARIANT/hm_churn $run_args $churn_args -d ${run_dir}/$VARIANT/
done
