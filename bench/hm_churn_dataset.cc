...
std::string key_file = "data/books_800M_uint64";
vector<uint64_t> all_keys;

void load_all_keys() {
  int buffer_size = 4096;
  uint64_t buffer[buffer_size/sizeof(uint64_t)];
  int keyFile = open(key_file.c_str(), O_RDONLY);
  uint64_t num_keys;
  read(keyFile, (void *)(&num_keys), sizeof(uint64_t));
  for (uint64_t i=0; i < num_keys; i = i + buffer_size/sizeof(uint64_t)) {
    read(keyFile, (void *)(buffer), buffer_size);
    for (int j=0; j < buffer_size/sizeof(uint64_t); j++) {
      all_keys.push_back(buffer[j]);
    }
  }
  printf("Loaded all keys!\n");
  close(keyFile);
}
...
void parseArgs(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "e:d:k:q:v:i:c:w:l:f:p:r:s:g:t:m:z:")) != -1) {
    switch (opt) {
		case 'd':
...
std::vector<hm_op> generate_churn_ops(
  ...
  if (mixed_workload) {
    abort(); // Not implemented with file from keys yet.
