...
// Big-endian code, rank from right to left.
...
// Returns the position of the rank'th 1 from right, ignoring the first `ignore` 1.
...
{
	return (METADATA_WORD(qf, tombstones, index) >> ((index % QF_SLOTS_PER_BLOCK) %
																								64)) & 1ULL;
}
...
static inline uint64_t find_first_tombstone(QF *qf, uint64_t from)
{
	...
	uint64_t tomb_offset = bitselect(b->tombstones[0] & (~BITMASK(slot_offset)), 0);
...
// shift the part of b (bend, bstart] to the left by amount, 
// big endian, right side is less significant. 
// if bstart == 0, shift left part of a into the right part of b.
...
static inline void qf_dump_block(const QF *qf, uint64_t i)
{
	...
	for (j = 0; j < QF_SLOTS_PER_BLOCK; j++)
		printf(" %d ", (get_block(qf, i)->tombstones[j/64] & (1ULL << (j%64))) ? 1 : 0);
	printf("\n");
...
static inline void shift_runends_tombstones(QF *qf, int64_t first, uint64_t last,
																 uint64_t distance)
{
	...
	if (last_word != first_word) {
		METADATA_WORD(qf, runends, 64*last_word) = shift_into_b(METADATA_WORD(qf, runends, 64*(last_word-1)), 
																														...
																														0, bend, distance);
		METADATA_WORD(qf, tombstones, 64*last_word) = shift_into_b(METADATA_WORD(qf, tombstones, 64*(last_word-1)), 
																														METADATA_WORD(qf, tombstones, 64*last_word),
																														0, bend, distance);
		bend = 64;
		last_word--;
		while (last_word != first_word) {
	...
	METADATA_WORD(qf, tombstones, 64*last_word) = shift_into_b(0, METADATA_WORD(qf,
																																					 tombstones,
																																					 64*last_word),
																													bstart, bend, distance);
																																										 ...
																																										 uint64_t		 noverwrites)
{
	...
	if (ninserts > 0) {
		...
		for (j = 0; j < ninserts - 1; j++)
			shift_runends_tombstones(qf, empties[j+1] + 1, empties[j] - 1, j + 1);
																																										 ...
																																										 uint64_t		 old_length)
{
	...
	uint64_t current_slot = overwrite_index + total_remainders;  // the slot to shift to
	uint64_t current_distance = old_length - total_remainders;  // the distance to shift
...
/* Set the range of the run [start, end)*/
static void run_start_end(const QF *qf, const size_t quotient,
                          size_t *const start, size_t *const end) {
  *start = run_start(qf, quotient);
  if (!is_occupied(qf, quotient))
    *end = *start;
  else
    *end = run_end(qf, quotient) + 1;
}
...
static int find(const QF* qf, 
  ...
  run_start_end(qf, quotient, run_start_index, run_start_index);
  *index = *run_start_index;
  if (*run_start_index == *run_end_index) return 0;
 ...
 */
static inline int _insert_ts_at(const QF *const qf, size_t index) {
  if (is_tombstone(qf, index)) return 0;
  uint64_t available_slot_index = find_first_tombstone(qf, index);
...
static inline int _insert(QF *qf, uint64_t key, uint64_t value,
  ...
  if (is_empty(qf, hash_bucket_index)) {
    ...
    ret_distance = _insert_ts_at(qf, insert_index);
    if (ret_distance == QF_NO_SPACE) return QF_NO_SPACE;
...
/* TODO: If tombstone_space == 0 and/or nrebuilds == 0, automaticlly calculate them
 * based on current load factor when rebuiding. */
uint64_t qf_init_advanced(QF *qf, uint64_t nslots, uint64_t key_bits, 
													uint64_t value_bits,
													uint64_t tombstone_space, uint64_t nrebuilds,
													enum qf_hashmode hash, uint32_t seed, void* buffer, 
													uint64_t buffer_len)
{
  ...
  float x = 20.;  // 1/(1-load_factor)
  if (tombstone_space == 0) tombstone_space = 2.5 * x;
  qf->metadata->tombstone_space = tombstone_space;
  if (nrebuilds == 0) nrebuilds = ceil(nslots / (1.5 * 2.5 * x));
  qf->metadata->nrebuilds = nrebuilds;
	qf->metadata->rebuild_slots = nslots / nrebuilds + 1;
	qf->metadata->rebuild_index = 0;
	qf->metadata->next_tombstone = qf->metadata->tombstone_space-1;
	qf->metadata->n_start_rebuild = nslots * 4 / 5;  // 0.8 load factor.

	// Set all tombstones
	char * b = (char *)(qf->blocks);
	size_t block_size = sizeof(qfblock)
	...
	pc_init(&qf->runtimedata->pc_nelts, (int64_t*)&qf->metadata->nelts, num_counters, threshold);
...
uint64_t qf_init(QF *qf, uint64_t nslots, uint64_t key_bits, uint64_t value_bits,
								 ...
								 buffer_len)
{
	return qf_init_advanced(qf, nslots, key_bits, value_bits, 0, 0, hash, seed, buffer, buffer_len);
}
...
bool qf_malloc_advance(QF *qf, uint64_t nslots, uint64_t key_bits, uint64_t
										 			...
										 			uint64_t tombstone_space, uint64_t nrebuilds)
{
	uint64_t total_num_bytes = qf_init_advanced(qf, nslots, key_bits, value_bits,
																		  				tombstone_space, nrebuilds, hash,
																							seed, NULL, 0);
	...
	uint64_t init_size = qf_init_advanced(qf, nslots, key_bits, value_bits, hash,
																				seed, tombstone_space, nrebuilds,
...
int64_t qf_resize_malloc(QF *qf, uint64_t nslots)
{
	...
	do {
		uint64_t key, value;
		qfi_get_hash(&qfi, &key, &value);
		qfi_next(&qfi);
		int ret = qf_insert(&new_qf, key, value, QF_NO_LOCK | QF_KEY_IS_HASH);
		if (ret < 0) {
...
int qf_insert(QF *qf, uint64_t key, uint64_t value, uint8_t flags)
{
  ...
  if (qf->metadata->noccupied_slots >= qf->metadata->n_start_rebuild) {
    size_t n_pushing = _rebuild(
      qf, &(qf->metadata->rebuild_index), qf->metadata->rebuild_slots);
  }
	...
	if (ret == QF_NO_SPACE || ret > DISTANCE_FROM_HOME_SLOT_CUTOFF) {
		...
		if (qf->runtimedata->auto_resize) {
			...
			if (ret == QF_NO_SPACE)
				...
				fprintf(stderr, "DISTANCE_FROM_HOME_SLOT_CUTOFF: %u.\n", ret);
			ret = QF_NO_SPACE;
		}
	}
	return ret;
								 ...
								 flags)
{
	...
	if (delta == 0)
		...
		ret = qf_insert(qf, key, value, flags);
	else
		ret = qf_remove(qf, key, flags);
...
void qf_merge(const QF *qfa, const QF *qfb, QF *qfc)
{
	...
	do {
		if (keya < keyb) {
			qf_insert(qfc, keya, valuea, QF_NO_LOCK | QF_KEY_IS_HASH);
			qfi_next(&qfia);
			qfi_get_hash(&qfia, &keya, &valuea);
		}
		else {
			qf_insert(qfc, keyb, valueb, QF_NO_LOCK | QF_KEY_IS_HASH);
			qfi_next(&qfib);
			qfi_get_hash(&qfib, &keyb, &valueb);
		}
...
void qf_multi_merge(const QF *qf_arr[], int nqf, QF *qfr)
{
	...
	while (nqf > 1) {
		...
		uint64_t values[nqf];
		for (i=0; i<nqf; i++)
			qfi_get_hash(&qfi_arr[i], &keys[i], &values[i]);
	...
	if (!qfi_end(&qfi_arr[0])) {
		...
		do {
			uint64_t key, value;
			qfi_get_hash(&qfi_arr[0], &key, &value);
			qf_insert(qfr, key, value, QF_NO_LOCK | QF_KEY_IS_HASH);
			qfi_next(&qfi_arr[0]);
			iters++;
 ...
 */
int _rebuild(const QF *qf, size_t *const from_run, size_t until_run) {
